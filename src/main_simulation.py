...
from geometry_msgs.msg import Pose, PoseStamped, Point, Transform
...
def run_simulation(system):
	...
	r = rospy.Rate(5)
	global display_traj_robot, robot_traj_robot, joint_traj_point_robot, joint_traj_robot, display_traj_walker, robot_traj_walker, joint_traj_point_walker, joint_traj_walker, time_run_start, target_object, total_error_position, total_error_orientation, total_time, env, object_params, object_params_sim, obss, r_max, phi, PAM_path_actual, object_path_actual, sys_mode, vis, bed_state, old_object_state, grasping_goal, PAM_goal, object_path_planned, found_solution, object_r, vis_bag, PAM_path_planned, desired_force, boolean, best_leg,  transition_mode, transition, blue_chair_state
	...
	if env.manipulation_object == "walker":
		...
		obss.append([env.blue_chair_state.x, env.blue_chair_state.y, env.blue_chair_state.phi, 0.6, 0.55])  #adds the blue_chair as an obstacle for target object
		obss.append([system.walker_state.x, system.walker_state.y, system.walker_state.phi, 0.54, 0.36])  #adds the walker as an obstacle for target object
	...
	obss_PAM.append([target_object.x, target_object.y, target_object.phi, object_params[7], object_params[8]])  #adds the target object as an obstacle for PAM
	...
	if (target_object.x-env.object_goal.x)**2+(target_object.y-env.object_goal.y)**2 <= 2* env.epsilon and (target_object.phi-env.object_goal.phi)**2 <= 2*env.epsilon_phi*10:
		sys_mode = 5
		desired = [0, 0]
		...
		if found_solution == 1 and not transition:
		          if current_leg == best_leg:
		                    ...
		                    if (phi_close >= env.phi_limit[1] or phi_close <= env.phi_limit[0]):
		                              ...
		                              # joint_traj_point_walker.positions = [copy.deepcopy(target_object.x), copy.deepcopy(target_object.y),copy.deepcopy(target_object.phi)]
		                              # joint_traj_point_walker.velocities = [0, 0, 0]
		                              # joint_traj_walker.points.append(copy.deepcopy(joint_traj_point_walker))
                                      #
		                              # joint_traj_point_robot.positions = [copy.deepcopy(system.PAM_state.x), copy.deepcopy(system.PAM_state.y),copy.deepcopy(system.PAM_state.phi)]
		                              # joint_traj_point_robot.velocities = [0, 0, 0]
		                              # joint_traj_robot.points.append(copy.deepcopy(joint_traj_point_robot))

		                              # Finding robot commands in local frame because it is less confusing
		                              phi_vel= np.arctan2(vel[1][1],vel[1][0])-system.PAM_state.phi
		                              if phi_vel < -np.pi:
		                                        phi_vel += 2*np.pi
		                              if phi_vel >= np.pi:
		                                        phi_vel -= 2*np.pi
		                              ...
		                              desired = [command[0][0], command[0][1], [force[0][0], force[0][1]]]
		                              object_path_planned = Path()
		                              object_path_planned.header.frame_id = 'frame_0'
...
if __name__ == '__main__':
	...
	cart_mu = [ 154.13239509, 183.27723826,   0.66815968,   0.39554305, 22.18162462, 23.85949298,  0.12067901, 0.9, 0.3]
	...
	try:
		...
		with open('/home/amir/Roya/sim_opt_experiment3_final.csv', mode='a') as file:
		          ...
		          for n_t in range(num_trial):
		                    ...
		                    for i in range(2500):
		                              ...
		                              if sys_mode == 5:
		                                        ...
		                                        break
		                              elif total_time>=180 or sys_mode ==0:
		                                        success_status = 0
		                                        total_error_position = np.sqrt((target_object.x-env.object_goal.x)**2+(target_object.y-env.object_goal.y)**2)
		                                        total_error_orientation = np.sqrt((target_object.phi-env.object_goal.phi)**2)*180/np.pi
		                                        rospy.loginfo("Fail :(")
